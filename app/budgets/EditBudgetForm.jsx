...
export default function EditBudget({ budget, setIsEditModalOpen, setToast }) {
  ...
  const [budgetMaximum, setBudgetMaximum] = useState(budget.maximum);
  const [budgetTheme, setBudgetTheme] = useState(budget.theme);

  const usedCategories = budgets
    .map((b) => b.category)
    .filter((c) => c !== budget.category);
  const usedThemes = budgets
    .map((b) => b.theme)
    .filter((t) => t !== budget.theme);
      ...
      <div className="flex flex-col gap-4">
        <div className="relative">
          ...
          <div ref={budgetRef}>
            ...
            >
              <p className="text-sm text-gray-900 ">{budgetCategory}</p>
              <span
                ...
                <ul className="py-1">
                  {categories.map((category, index) => (
                    <li
                      key={index}
                      onClick={() => (
                        setBudgetCategory(category), setIsCategoryOpen(false)
                      )}
                      className={`py-3 hover:bg-neutral-200 cursor-pointer text-gray-900 text-sm ${
                        budgetCategory === category ? "font-bold" : ""
